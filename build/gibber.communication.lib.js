...
//                 }, true, true )
  ...
  parse: function( msg ) {
    ...
    switch( msgType ) {
      case 'noteon' :
        if( MIDI[ targetInput ].channel[ channel ].target ) {
          MIDI[ targetInput ].channel[ channel ].target.note( MIDI.ntof( num ) , value / 127 )
        }
        break;
      case 'noteoff' :
        if( MIDI[ targetInput ].channel[ channel ].target ) {
          MIDI[ targetInput ].channel[ channel ].target.note( MIDI.ntof( num ) , 0 )
        }
        break;
      case 'cc' :
				MIDI[ targetInput ].channel[ channel ].cc[ num ].value( value )
        break;
    }
